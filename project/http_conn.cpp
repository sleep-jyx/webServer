...
//网站资源根目录
const char *doc_root = "/home/jyx/文档/unix/webServer/project/root";
...
{
    printf("--debug--:调用进程处理cgi\n");
    if (fork() == 0)
    {
        printf("--debug--:子进程dup2(),%s\n", m_real_file);
        int ret = dup2(m_sockfd, STDOUT_FILENO);
        ...
        //if (ret < 0)
        //    printf("--debug--:dup2()error\n");
        // else
        //   printf("--debug--:dup2()成功？%s\n", m_real_file);
